...
namespace Xamarin.Services.Permissions
{
...
#endif
	{
		...
		public Task<PermissionStatus> CheckPermissionStatusAsync(Permission permission)
		{
			...
			var targetsMOrHigher = context.ApplicationInfo.TargetSdkVersion >= global::Android.OS.BuildVersionCodes.M;

			foreach (var name in names)
			{
				if (targetsMOrHigher)
				{
					if (ContextCompat.CheckSelfPermission(context, name) != global::Android.Content.PM.Permission.Granted)
				...
				lock (locker)
				{
					if (permission == Permission.Microphone)
					{
						if (!results.ContainsKey(Permission.Speech))
							results.Add(Permission.Speech, grantResults[i] == global::Android.Content.PM.Permission.Granted ? PermissionStatus.Granted : PermissionStatus.Denied);
					}
					else if (permission == Permission.Location)
					{
						if (!results.ContainsKey(Permission.LocationAlways))
							results.Add(Permission.LocationAlways, grantResults[i] == global::Android.Content.PM.Permission.Granted ? PermissionStatus.Granted : PermissionStatus.Denied);

						if (!results.ContainsKey(Permission.LocationWhenInUse))
							results.Add(Permission.LocationWhenInUse, grantResults[i] == global::Android.Content.PM.Permission.Granted ? PermissionStatus.Granted : PermissionStatus.Denied);
					}

					if (!results.ContainsKey(permission))
						results.Add(permission, grantResults[i] == global::Android.Content.PM.Permission.Granted ? PermissionStatus.Granted : PermissionStatus.Denied);
				}
			}
		...
		bool HasPermissionInManifest(string permission)
		{
			try
			{
				...
				var info = context.PackageManager.GetPackageInfo(context.PackageName, global::Android.Content.PM.PackageInfoFlags.Permissions);
